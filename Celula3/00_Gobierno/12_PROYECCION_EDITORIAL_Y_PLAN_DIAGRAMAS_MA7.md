...
**Fecha:** 2026-09-06
**Estado:** `MA-7 COMPLETADA — PUERTA P4 APROBADA EN MA-8`
**Alcance:** propuesta de estructura y figuras para el Subdocumento 4 del Informe 1; todavía no constituye la redacción final ni contiene diagramas terminados.
...
### 5.1 Set obligatorio recomendado — cinco figuras

| Figura | Tipo/vista | Pregunta única | Contenido obligatorio | Qué se excluye para evitar saturación | Criterio de aceptación |
|---|---|---|---|---|---|
...
Estas cinco figuras son el set recomendado. No se producirán diagramas separados de microservicios, observabilidad, CI/CD, cada ADR, cada actor ni cada amenaza.
...
| Dominio/flujo | Fuente de verdad normal | Autoridad durante corte | Almacén | Protección | Retención/respaldo | Dueño del detalle |
|---|---|---|---|---|---|---|
| operación/gate/patio/nave | según dominio y fase A3 | núcleo local para cinco funciones críticas | `DATA-CORE` | IAM + cifrado + evidencia | resumen de plazos aplicables | Subdoc. 5 |
| reefer/telemetría | instrumentación validada | local para alarma/serie crítica | `DATA-TS` | segregación + integridad | 5 años reefer; granular según catálogo | Subdoc. 5 |
| documentos/evidencia | contexto productor + `SRV-EVID` | temporal local cuando aplica | `DATA-DOC` | firma, hash, cifrado y custodia | según clase | Subdoc. 5 |
| analítica/emisiones | derivados trazables | diferido durante corte | `DATA-AN` | acceso por rol y linaje | según dato fuente | Subdoc. 5 |
...
| `V-DATA-01` | vista arquitectónica de datos | 4 dominios; detalle en Subdoc. 5 |
...
10. F1–F5 deben coincidir con AWS `sa-east-1`/`us-east-1`, 72 h, 8 h, ≤90 min, RTO/RPO, catálogos y T-11 vigentes.
...
MA-7 deja aprobables:

- la estructura 4.1/4.2;
- el presupuesto de extensión;
- cinco figuras obligatorias;
- la vista de datos como tabla;
- una sexta figura condicional;
- las tablas publicables y el contenido que no se trasladará.

Después de aprobación P4, MA-8 ajusta la secuencia de producción y control:

1. redactar el consolidado y producir F1–F5 desde las mismas fuentes; activar F6 solo si la legibilidad lo exige;
2. cruzar `V-DATA-01`, ensamblar T-11 y figuras;
