...
- `CTX-OPS` — Núcleo operacional: agregado maestro de contenedores, movimientos, posición y hechos operacionales. Fuente de verdad para el estado del contenedor dentro del terminal. Registra además la **detención de grúa de muelle atribuible a espera de tractocamión** como evento propio, con inicio, término y equipo involucrado (`RF-TRA-04`, Crítica): es un hecho operacional derivado de cruzar el estado de la grúa con la disponibilidad del equipo asignado por `CTX-YARD`, no un sensor nuevo. El `CP, Cap. 4.3` identifica esa espera como **la causa de la brecha de productividad** —24,8 movimientos por hora frente a los 30 que la alianza naviera exige desde 2029—, y su criterio de aceptación es que las detenciones registradas reconcilien con la diferencia entre tiempo total de atención y tiempo de operación efectiva, sin residuo no clasificado. Es el insumo que hace calculable la productividad que `CTX-VESSEL` reporta.
...
- `CTX-YARD` — Patio y posición: asignación de posiciones (bahía-fila-nivel), verificación cruzada con telemetría de equipos, **posicionamiento y asignación de los 42 tractocamiones al movimiento de grúa, con tiempo estimado de arribo y reasignación ante indisponibilidad** (`RF-TRA-01`, `RF-TRA-02`, `RF-TRA-03`, `RF-TRA-05` y `RF-TRA-06`; hoy el supervisor los asigna por radio y su ubicación *«es conocimiento del supervisor, no del sistema»* — `CP` línea 215), gestión de condiciones dinámicas (pilas, pesos, tipos), **programación anticipada de remociones** (`RF-PAT-10` — ataca el 18 % de remociones evitables de forma proactiva, no solo reactiva, y es la base física que `CTX-INSP` usa para agendar inspecciones con margen) y cálculo de remociones. Meta: 100 % de posiciones conocidas correctas y ≤0,5 % "por verificar" al cierre (Maestro §4.8).
...
| 2 | Posición/inventario y cruce de zonas | `CTX-YARD`, `CTX-OPS`, `INT-TOS` | Asignación y verificación de posición bahía-fila-nivel; traspaso de autoridad `dominio × zona × fase` con el TOS | Se posterga la conciliación **hacia la nube**; la de plataforma ↔ TOS 2012 **continúa localmente** —ambos están en la sala, `C1 §4`— y conserva su ventana de investigación de 48 h, que es más corta que el corte y por eso no puede depender del enlace (`A3 §4`) | Maestro §9.1, §8; posición ≤30 s |
...
| `CTX-OPS` | Núcleo operacional | 4-Negocio | Agregado contenedor, movimientos, posición y hechos operacionales, incluida la **detención de grúa por espera de equipo** (`RF-TRA-04`) | Crítica |
| `CTX-GATE` | Gate y citas | 4-Negocio | Prevalidación, citas, cola virtual, entradas/salidas y excepciones | Crítica |
| `CTX-YARD` | Patio y posición | 4-Negocio | Asignación, verificación cruzada, condiciones dinámicas, remociones y **posicionamiento y asignación de tractocamiones** (`RF-TRA-01`, `RF-TRA-02`, `RF-TRA-03`, `RF-TRA-05`, `RF-TRA-06`) | Crítica |
