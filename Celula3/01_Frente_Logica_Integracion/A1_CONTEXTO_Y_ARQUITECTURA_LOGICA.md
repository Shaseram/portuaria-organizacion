...
| `SRV-IAM` como única autoridad de identidad | Sin `SRV-IAM` no hay autenticación nueva ni revocación | Se acepta porque las sesiones activas se cachean localmente (`EDGE-RUN`, Parcial — §3.2) y el requisito RT-02.09 exige degradación informada, no cero autoridad | Alta disponibilidad e IAM redundante (D1); la baja efectiva de `RT-12.10` se cuenta **desde la desvinculación** y debe poder ejecutarse localmente durante el corte | `SPOF-10` `SRV-IAM`, identidad |
...
>
> Resolución adoptada el 2026-09-06: **un solo término, un solo dueño.** C1 §5 adopta los valores de esta tabla y no los reescribe; si necesita expresar redundancia física, esa columna lleva su propio nombre. Las seis diferencias se cerraron adoptando el valor de A1 (`F2-COR-006`). Queda **una abierta**: `SRV-IAM`. C1 argumenta que sin autenticación local durante las 72 h las cinco funciones críticas no se operan, lo que por la definición de arriba es la definición de *Crítica*; la propuesta de elevarla está escalada como `F2-ESC-017` y se resuelve en la Puerta 2 junto con `ADR-008`. Mientras tanto conserva `Alta` en este catálogo, que es la fuente.
...
| `SRV-IAM` | Identidades, roles, sesiones, políticas | Todas las capas | Parcial — sesiones y credenciales activas cacheadas localmente para validación offline; alta de identidades nuevas y sincronización de revocación con el directorio corporativo quedan pendientes hasta reconectar. **El plazo de baja de `RT-12.10` se cuenta desde la desvinculación, no desde la reconexión**: la revocación debe poder ejecutarse localmente dentro de esas 24 h | SRV-IAM |
