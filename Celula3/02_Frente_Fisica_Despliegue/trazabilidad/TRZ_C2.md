...
| `TRZ-C2-038` | `RT-03.01` proveedor y regiones · `RT-15.04`/`.05` intensidad de carbono y análisis comparativo · `RT-03.02` multi-AZ · `RT-03.07` reversibilidad · D2 `B7.2 #8` — «`ADR-011` no compara alternativas concretas» · `THR-073`/`SPOF-22` | — | nube | **espacio de decisión acotado:** región primaria `A1-CL`/`A2-SA`/`A3-MIX`; región secundaria `B1-MISMO`/`B2-OTRO`/`B3-HIBRIDO`. Recomendación condicionada `A1-CL`+`B1-MISMO` **con** respaldo inmutable fuera del plano de control primario | faltan latencia medida, intensidad de carbono por región y catálogo de servicios: no se selecciona | — | `T11-C2-17`, `18`; condición de `T11-C2-12` | **PARA DECISIÓN DEL INTEGRADOR** (`F2-ESC-009`) |
| `TRZ-C2-039` | D2 `B7-O01` — `SPOF-13` recuperabilidad del respaldo y `SPOF-22` dominio de fallo común del proveedor **no deben consolidarse** · `RT-07.10`/`.11`/`.12` | — | respaldo | se conservan separados: `T11-C2-12` custodia de medios y `T11-C2-18` región secundaria siguen siendo ambas necesarias | ejercicio regional y de salida que mida RTO/RPO y dependencias comunes | — | `T11-C2-12`, `18` | PARA REVISIÓN |
| `TRZ-C2-040` | `RT-03.16` — el monitoreo on-premise se integra a **la misma** plataforma que la nube, sin puntos ciegos · D2 `B6-F05` | — | observabilidad | **corrección:** `T11-C2-19` y `SEC-LOG-01`/`SEC-SIEM-01` son una sola plataforma; `T11-SEC-04` no genera fila propia | control 1:1 del T-11 en la Puerta 2 | — | `T11-C2-19` | PARA REVISIÓN |
