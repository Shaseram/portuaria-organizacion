...
| `F2-ESC-004` | 2026-09-05 | C2 | CITA INCOMPLETA | rango `RT-06` truncado en `.24`; el BTT exige hasta `.34` | se pierden custodia de medios, espacio de operación y rutas de comunicaciones | corregir el rango en el Maestro §10.3 | C2, C4, T-11, `ADR-005` | **CERRADO 2026-09-06** — Maestro §10.3 corregido a `RT-06.34` |
| `F2-ESC-005` | 2026-09-05 | todos | OMISIÓN DE ALCANCE | checklist del Capítulo C del BTT no incorporado | 28 entregables exigibles con su artículo y su Sobre; 6 son del Frente 2 | agregarlo como control en la matriz global §2 | los tres frentes | **CERRADO 2026-09-06** — Matriz Global §6, los 28 entregables con dueño y estado |
...
| `F2-COR-012` | 2026-09-06 | C3 | CORRECCIÓN PROPIA | los candidatos T-11 de C3 no tenían identificador | C4 §9 y `TRZ_C4` citaban `T11-C3-01` y `T11-C3-03` como si existieran; C3 §13 los enumeraba en prosa y eran cinco, no tres. Dos filas del control 1:1 no volvían a ninguna parte | publicada la tabla `T11-C3-01..04`. Al ponerles ID aparecieron **dos duplicados**: la custodia con verificación de restauración ya era `T11-C2-12`, y el gestor de secretos ya era `T11-SEC-03`. Declarados, no arrastrados | C3, C4, T-11 | APLICADO |
| `F2-INT-005` | 2026-09-06 | gobierno | CAMBIO DE ROL | el equipo designa a este frente como **integrador** | hasta aquí `00_Gobierno/` y `90_Consolidado/` no se editaban por la regla de autoridad del README. Con el rol asignado, la restricción deja de aplicar y las brechas de gobierno pasan a ser trabajo propio | aplicadas en el Maestro §1.1, §9.2.1 y §10.3, y en la Matriz Global §5 y §6. Se cierran `F2-ESC-003` a `007` y `015` | todos | **APLICADO** |
...
**Lo que se devuelve a Frente 3.** `F2-ESC-017` quedó **cerrado**: A1 elevó `SRV-IAM` a `Crítica` el 2026-09-06, de modo que la capacidad local de identidad es ahora obligación de continuidad y no una mitigación — eso acota el alcance de `ADR-008` y D1 debe recogerlo. `F2-ESC-015` y `F2-ESC-016` siguen abiertos hacia D2. Y el traspaso de los cuatro puntos de falla de `F2-ESC-014` al registro consolidado sigue pendiente en D2.
