...
| `SRV-IAM` | Alta → **Crítica** *(elevada el 06-09)* | **crítica** | **se resuelve dando la razón a C1**: A1 eleva el valor, con partición dual | único caso donde C1 era **más** exigente que A1, y con razón física; ver abajo |
...
**`SRV-IAM`: aquí C1 no se alineó en silencio, y se le dio la razón.** Si durante las 72 horas nadie puede autenticarse, las cinco funciones críticas no se operan aunque el cómputo esté vivo: el corte de enlace se convierte en corte de operación. Ese es el motivo por el que la matriz le asigna caché de sesión en `PHY-OPS-01` y por el que C2 marca `SEC-IAM-01` como requisito excluyente de capacidad local. Por la escala de A1, «debe sobrevivir 72 h e ir en `EDGE-RUN`» es la definición de *crítica*. **Resuelto el 2026-09-06: A1 eleva `SRV-IAM` a `Crítica`**, con la misma partición dual que `CTX-VESSEL` — crítica en la autenticación y autorización de identidades vigentes, alta en el alta de identidades nuevas y la sincronización con el directorio, que esperan al enlace. `F2-ESC-017` queda cerrado.

**Qué cambió y qué no.** El dimensionamiento no cambia: `PHY-OPS-01` ya alojaba la caché de sesión, `T11-SEC-02` ya declaraba capacidad local y el volumen de una caché de credenciales vigentes es despreciable frente a los ≈183 GB útiles de §6.1 de C4. Lo que cambia es **qué se promete**: la autenticación local deja de ser una mitigación que ofrecemos y pasa a ser parte del compromiso de continuidad de 72 horas, y `SRV-IAM` entra en la lista de componentes que `EDGE-RUN` replica. Para D1 la consecuencia es directa: la capacidad local de identidad se vuelve **obligación**, no opción, y eso acota el alcance de `ADR-008`.
