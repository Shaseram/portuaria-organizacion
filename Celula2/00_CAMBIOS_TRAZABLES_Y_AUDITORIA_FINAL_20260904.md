...
| **B4** | Se crea **`RN-11`** (tolerancia de desviación de temperatura) y se enlaza al criterio de `RF-REF-04`. Registro de **10 → 11 reglas** | `02_…/Registro reglas de negocio v2.md` · `…parte2.md` | El criterio de `RF-REF-04` manda simular «una desviación», pero ninguna regla definía la **banda** ni la **duración** que la constituyen: `RT-05.29` fija el plazo de la alarma, no el umbral que la dispara. La regla **no fija valores numéricos** —el caso no los entrega— y acota la duración mínima para que la alarma siga cabiendo en los 5 minutos |
...
| **B11** | Se retira la meta de negocio del criterio de aceptación de `RF-NAV-12`, `RF-NAV-03` y `RF-INS-07` | `01_Requerimientos/…parte3.md` | CP, Cap. 17.1: «Un requerimiento no es una frase copiada de este documento. «*El camión debe demorar cuarenta y cinco minutos*» no es un requerimiento: **es un resultado esperado**». Lo que el capítulo descarta es la **meta de negocio usada como requerimiento**, no el campo «resultado esperado» de la ficha, que el mismo capítulo exige. Las metas retiradas ya viven en la tabla de indicadores del numeral 11.2 |
...
|---|---|
| «Faltan 7 RNF de los parámetros del Cap. 15» | El hallazgo suponía que cada parámetro del Cap. 15 debe existir como RNF. El CP, Cap. 17.1 pide que el catálogo de RNF «incorpore los parámetros del Capítulo 15», pero el Cap. 17.2 deja **deliberadamente sin resolver** los casos limítrofes y ofrece como ejemplo justamente la alarma reefer de cinco minutos. Seis de los siete estaban cubiertos fuera de `RNF.md`, la mayoría como criterio de aceptación de un RF. Lo que corresponde no es duplicarlos, sino **declarar el criterio de clasificación** — ver C7 |
...
- **Etapa de `RF-POR-09`**: se confirma **Etapa 2**, con el argumento corregido. `RF-INT-02` también es de Etapa 2, de modo que los dos flujos del criterio 13 entran en producción en el mes 21 y el indicador se mueve como una sola cosa; además la precondición de `RF-POR-09` es `RF-POR-02`, que es de Etapa 2. El argumento original —«coherencia con el dominio»— era inválido: el catálogo admite excepciones por requerimiento, como `RF-NAV-09`, `RF-APP-01` y `RF-FIR-01`. El reparto 82 / 57 ya lo refleja.
...
# Anexo — Tercera ronda · auditoría de la propia corrección · 2026-09-05

> Esta ronda no revisó el material original: revisó **los cambios B1–B13** de la ronda anterior, buscando lo que la corrección hubiera roto o afirmado de más. Se levantaron nueve hallazgos; **siete se confirmaron abriendo el archivo y dos se descartaron**. Los siete están aplicados aquí.

## Correcciones aplicadas

| ID | Qué estaba mal | Corrección | Archivo |
|---|---|---|---|
| **C1** | **B4 se aplicó al requerimiento equivocado.** El enlace a `RN-11` y su nota de corrección quedaron en `RF-REF-05` («detección de desconexión de toma»), no en `RF-REF-04`. La nota además afirmaba que el criterio «solo verificaba desconexión», cuando `RF-REF-04` ya decía «se simula una desviación sobre una muestra de 30 tomas» | Se revierte la adición en `RF-REF-05` y `RN-11` se invoca desde el criterio de `RF-REF-04`. El fundamento se reescribe: el vacío real es que **«desviación» no estaba definida** —ninguna regla fijaba banda ni duración—, no que la rama no se probara | `…parte2.md` |
| **C2** | **El criterio de `RF-REF-07` era insatisfacible.** Exigía alarmar «tras 3 intervalos de muestreo consecutivos **y**, en todo caso, dentro de los 5 minutos», pero `RF-REF-01` admite muestreo de hasta 5 minutos: sobre 1 min 40 s los tres intervalos ya no caben en el techo | Las dos condiciones pasan a operar como **lo que ocurra primero** | `…parte2.md` |
| **C3** | **B2 inventó una descomposición del 41 %.** El numeral 11.2 quedó con dos filas, cada una con base «fracción del 41 %». El CP, Anexo A atribuye ese 41 % **íntegro** al flujo «Embarcador o agencia → Documentación»; el caso no declara cifra para la pata naviera → terminal. La propia nota de `RF-INT-02` decía esto y la tabla lo contradecía | El 41 % queda completo en la fila de `RF-POR-09`. La fila de `RF-INT-02` pasa a **«sin cifra declarada en el caso»**, conservando su meta. Se corrige también el **Origen** de `RF-INT-02` | `…parte3.md` |
| **C4** | **Cita elidida del CP, Cap. 17.1.** El fundamento de B11 citaba «un requerimiento no es una frase copiada… **es** un resultado esperado», que leído así afirma lo contrario del original y choca con el mismo capítulo, que exige el campo «resultado esperado» en cada ficha | Se restituye la cita completa y se explicita qué descarta el capítulo: la **meta de negocio usada como requerimiento**, no el campo de la ficha | este anexo |
| **C5** | El estado de Célula 2 en la narrativa seguía declarando **«Catálogo de RNF — 84»** tras el alza a 91 de B1 | Corregido a **91** | `04_Narrativa_Subdoc3/…` |
| **C6** | **`RN-11` no acotaba la «duración mínima»**. Siendo parametrizable sin techo, un valor sobre 5 minutos hacía inalcanzable el criterio de `RF-REF-04`, que se mide contra `RT-05.29` | La regla acota la duración mínima admisible al plazo de `RT-05.29` y obliga a **rechazar la parametrización** que lo haga inalcanzable | `02_…/Registro reglas de negocio v2.md` |
| **C7** | **El descarte del hallazgo de los 7 RNF estaba sobredicho.** Decía que «ninguna base exige que un parámetro del Cap. 15 sea RNF», pero el CP, Cap. 17.1 sí pide que el catálogo de RNF «incorpore los parámetros del Capítulo 15». La tensión real está entre ese párrafo y el Cap. 17.2, que deja los casos limítrofes abiertos | Se reescribe el descarte y se **declara el criterio de clasificación** en el numeral 1.2 del catálogo: el parámetro que califica una conducta ya descrita va al catálogo de RNF; aquel del que depende que la conducta exista va como umbral del criterio de aceptación del RF. Sin duplicación, y con cierre en el T-12 | `…parte1.md` · este anexo |

## Hallazgos de esta ronda descartados tras verificación

| Hallazgo | Por qué no aplica |
|---|---|
| «Las metas retiradas por B11 siguen huérfanas en la trazabilidad» | El numeral 11.2 **es** el destino que B11 declara para ellas. Las otras apariciones (`MC-24`, registro del Plan Maestro) son registros históricos fechados de la ronda anterior, no afirmaciones vigentes |
| «`RF-ACC-01` atribuye mal los ~2.100 eventuales al Cap. 2.4» *(reincidente)* | Ya descartado en la ronda anterior y verificado de nuevo contra la línea 477 de `…parte2.md`: la cita es correcta. **No reabrir** |

## Conteos

Sin cambios respecto de la segunda ronda: **139 RF · 91 RNF · 11 reglas · 21 decisiones · 25 supuestos**, reparto **82 / 57**. Esta ronda no crea ni elimina requerimientos.
