...
> **Autoría:** Decisiones N° 2 a 20 y supuestos A a L, Isidora Cisternas. Decisión N° 21, metas, supuestos M a T y correcciones de Fase 4, Rodolfo Fernández.
...
> **Corrección vigente:** 2026-09-04, aplicada contra el Plan Maestro y trazada en `../03_Trazabilidad_y_Bases/registro_correccion_plan_maestro_20260904.md`.
...
| **C** | **Supuestos declarados** — de calendario, de interpretación, metodológicos y de alcance | **25** |
...
| 6 | Sistema de citas: obligatoriedad y penalización | Sí — LA/LB, Vancouver, Port Botany, Arica (Ramírez-Nafarrate et al. 2017) | Sí | `RF-GAT` |
...
| **20** | Destino de la sala de servidores | Sí — comparación de tres alternativas y ambiente marítimo | **No** — arquitectura física | reflejo en `RF-OPD` |
...
> **Fundamento adicional (incorporado desde el cruce con Célula 1, Cap. 2.8):** un reemplazo integral, incluso por fases, concentraría un nivel de esfuerzo incompatible con las demás urgencias del terminal y excedería la capacidad de absorción del equipo de TI local — **la Restricción no negociable N° 11 fija esa área en 5 personas**. El patrón de envolvimiento no es solo la opción técnicamente más prudente: es la única compatible con la dotación real que tendrá que sostener la transición.
 
**Supuestos que aporta:** S1 a S5, recogidos en el bloque C.
**Requerimientos que introduce:** los catorce de `RF-CON`, incluidos el flujo legado→nuevo y la autoridad única por zona durante la coexistencia.
...
| 3 | **«El algoritmo asigna, el operador ejecuta»**, sin paso de confirmación intermedio. Complementada por el supuesto de la vía de excepción con interlock (ver C, supuesto O) |
| 4 | **«El algoritmo propone, el planificador aprueba o corrige, y toda corrección queda registrada con su motivo».** La captura del conocimiento tácito del planificador es un frente propio de la Etapa 1, independiente del módulo de software y anterior a él: arranca en el **mes 1**, primera versión del registro **antes del hito H2 (mes 4)**, versión validada con el planificador **antes del mes 12** — resolución completa en `01_decision_01_tos_2012_registro_final.md`, Pendiente 6, que además identifica este frente como el mismo **Registro de Reglas de Negocio** que ya exige el Cap. 17.1 (ver RN-01) |
...
| 6 | Sistema de citas **opcional, sin multa, con prioridad de atención** para quien cumple su cita, complementado con una **cola virtual**: el sistema notifica proactivamente al transportista cuando su ventana se retrasa, para que el camión demore su salida desde origen en vez de esperar en la vía pública cercana al terminal. Evoluciona a obligatorio solo para transportistas de alto volumen recurrente, una vez validado |
...
| 18 | Adopción de **BAPLIE, COPRAR, COARRI y CODECO** (UN/EDIFACT, mantenidos por SMDG). El canal puente asistido queda limitado a navieras no pertenecientes a la alianza; desde la vigencia de la exclusividad en 2029 las tres navieras de la alianza deben operar integradas, sin redigitación ni canal puente |
...
| 20 | La ubicación definitiva del cómputo local se resolverá mediante un ADR comparando tres alternativas: **(a)** remediar y endurecer la sala actual, **(b)** reemplazarla o reconstruirla dentro del terminal, y **(c)** mantener un edge local mínimo con nube. Ninguna alternativa podrá eliminar la autonomía local de 72 horas ni presumir infraestructura fuera del ambiente portuario que las bases no declaran |
...
| 2 | Exactitud y resolución de la posición | 3,1 % registrados donde no están | **100 % de posiciones conocidas correctas** y **≤ 0,5 % de posiciones por verificar no resueltas al cierre del turno** | Cierre de validación y transferencia de autoridad de Etapa 1 | Se separan dos métricas que no son equivalentes: una posición declarada «conocida» debe ser correcta; la tolerancia de 0,5 % solo aplica a excepciones explícitamente marcadas «por verificar» y pendientes al cierre. La meta requiere verificación cruzada de `RF-PAT-02` y ciclo de `RF-PAT-04`; DGPS por sí solo no basta |
...
| 14 | Diferencial de prioridad por cita cumplida | **≥ 30 % más rápido** | `RF-GAT-02` | La adopción decide el resultado del sistema de citas: Giuliano y O'Brien (2007) documentan efecto nulo en Los Ángeles y Long Beach con adopción de 5 a 30 %. Un incentivo imperceptible no genera adopción. **Evidencia complementaria:** Ramírez-Nafarrate et al. (2017), sobre el Puerto de Arica, muestran que el sistema sostiene beneficios incluso con cumplimiento parcial —a 75 % de llegadas dentro de ventana, la espera promedio se mantiene bajo 10 min—, lo que respalda no exigir cumplimiento perfecto vía multa, siempre que la prioridad ofrecida sea perceptible |
| 15 | Instrucciones de embarque digitadas | **0 % para las tres navieras de la alianza desde la vigencia de la exclusividad en 2029; ≤ 5 % solo para otras navieras** al cierre de Etapa 2 | `RF-INT-02` | La alianza es indivisible con integración estándar, cero redigitación y vigencia 2029; el residual solo puede corresponder a navieras ajenas a la alianza en canal puente temporal |
...
# Bloque C — Los 25 supuestos declarados
 
## C.1 Supuestos de calendario y de la Decisión N° 1
 
| ID | Supuesto | Fundamento | Impacto si resulta equivocado | Validación |
|---|---|---|---|---|
| **S1** | El mes 1 del contrato corresponde a **febrero de 2027** | Entrega de resultados el 01-12-2026 (BA, Formulario T-20) más dos meses para firma del contrato y constitución de garantías | Se desplaza todo el calendario; si el desplazamiento supera cuatro meses, alguna puesta en producción entra en congelamiento | Firma del contrato |
| **S2** | Es factible **interceptar y escribir** hacia el sistema de 2012 desde una capa externa | Es condición del patrón de envolvimiento progresivo; advertido como tal por Microsoft (2025a) en la Decisión N° 1 | Se anula la reversibilidad y debe replantearse la Decisión N° 1 completa | **Puerta de decisión del mes 4** |
| **S3** | Durante el congelamiento solo serían admisibles validaciones **paralelas, no invasivas, de solo lectura y sin autoridad operacional**, y únicamente si el CLIENTE confirma formalmente que no constituyen una intervención prohibida | Permite observar volumen real sin instalar, desplegar, migrar, corregir producción ni ejecutar cutover — ninguna de las conductas que la restricción no negociable N° 9 prohíbe | Si el CLIENTE no confirma esta interpretación, toda validación afectada se replanifica fuera del período; no se fuerza el calendario sobre una restricción no negociable | Consulta C1 antes de aprobar el cronograma base |
| **S4** | El patio admite **segmentación en bloques operativos** para el barrido de verificación | Práctica habitual de verificación de inventario y coherente con la referencia al bloque C que hace el CP, Cap. 8 | Debe replantearse el método de verificación física del inventario | Levantamiento |
| **S5** | El soporte del fabricante **puede vencer tan pronto como el 01-01-2028**; el plan se evalúa contra ese escenario | Supuesto deliberadamente conservador. BA, Art. 5.3 y 5.4 obligan a adoptar la interpretación más exigente ante ambigüedad. Ver §15.6 de la Decisión N° 1 | Existe una ventana de exposición **estructural del calendario contractual**, común a las tres opciones, declarada y mitigada | Consulta C2 |
...
| ID | Supuesto | Fundamento | Impacto si resulta equivocado | Validación |
|---|---|---|---|---|
| **A** | La operación desconectada de 72 h y las 8 h de terminales se clasifican como **Disponibilidad**, no como requisito funcional ni como decisión de arquitectura | El CP, Cap. 17.2 ofrece este caso como limítrofe y evalúa la consistencia del criterio propio, no la coincidencia con una respuesta. Se aplica el criterio declarado en §1.2 del catálogo de RF: el umbral califica un comportamiento ya descrito, no lo constituye. | Si la Comisión los espera como requerimientos funcionales de operación degradada, la conducta ya está descrita en `RF-OPD` y solo cambia la ubicación del umbral: no se pierde cobertura. | Caso limítrofe que el CP, Cap. 17.2 deja abierto. Ver los requerimientos en revisión, bloque E |
| **B** | «Sin rediseño» se interpreta como **ausencia de valores de sitios y bloques codificados en duro**, verificable por revisión de arquitectura y ADR | Las bases no definen método de verificación para «sin rediseño» (BTT RT-09.03; CP, Cap. 15, RT-02.12). La ausencia de valores codificados en duro es la única lectura verificable por revisión de arquitectura. | Si el CLIENTE espera evidencia adicional —por ejemplo un ADR específico de extensibilidad— se agrega el artefacto sin alterar el umbral. | Levantamiento |
| **C** | Las restricciones cualitativas —sin ventana de detención, prohibición de intervenir— se traducen en umbral **«0 ventanas/intervenciones»** | El CP, Cap. 17.1 exige umbral numérico para todo RNF, y una prohibición absoluta solo admite el umbral cero. | Si el CLIENTE admite excepciones acordadas, el umbral pasa de «cero» a «cero no autorizadas» y cambia el método de verificación, no el requisito. | Confirmar con arquitectura al definir la estrategia de despliegue sin interrupción |
| **D** | Umbral **«100 % de alarmas con confirmación registrada»** para el canal redundante | CP, Cap. 15, RT-16.21 exige confirmación de recepción por una persona identificada; sin umbral de 100 % la exigencia sería inverificable. | Si la dotación del turno de madrugada no permite sostener el 100 %, el compromiso se traslada al escalamiento automático y debe redimensionarse el modelo de operación. | Revisar con el jefe de operaciones la dotación real del turno de madrugada |
| **E** | Escala de prioridad propia **Crítica / Alta / Media**, distinta de «Obligatorio/Deseable/Según caso» de las bases | Las bases clasifican por carácter de exigibilidad —Obligatorio, Deseable, Según caso— y no por prioridad de implementación. Son dos dimensiones necesarias y no equivalentes. | Nulo sobre la exigibilidad, que la fija el carácter en las bases. El riesgo es de lectura: una prioridad Media sobre un requisito obligatorio puede interpretarse como postergable, por lo que el Formulario T-12 debe mostrar el carácter junto a la prioridad. | Revisión cruzada del equipo. **Aplicada también al catálogo de RF** |
| **F** | Convención de identificadores **`RNF-<CAT>-##`** | Ninguna base fija formato de identificador; la convención se alinea con la ya usada en el catálogo funcional. | Nulo en contenido. Un cambio de convención obligaría a rehacer todas las referencias cruzadas del catálogo y de la trazabilidad. | Alineada con `RF-<DOM>-##` en el catálogo funcional |
| **G** | Se asume la existencia de **herramienta de monitoreo (APM) y tablero operacional** como método de verificación | BTT, RT-14.01 y RT-14.02 ya exigen observabilidad unificada y tableros accesibles al CLIENTE: el método de verificación se apoya en una capacidad que las bases obligan a construir. | Si la arquitectura del Subdocumento 4 no la provee, todos los métodos de verificación que dependen de ella deben sustituirse por medición manual, lo que encarece la operación. | Alinear con la arquitectura del Subdocumento 4 |
| **H** | Umbral **«0 rutas cruzadas no autorizadas»** para la segregación efectiva de redes | CP, Cap. 10, restricción no negociable N° 6 exige segregación efectiva; el único umbral compatible con una restricción no negociable es cero. | Si la métrica defendible resulta otra —por ejemplo ausencia de dominio de conmutación compartido— cambia la evidencia, no la obligación. | Confirmar con arquitectura la métrica más defendible |
| **I** | Umbral **«0 interfaces que incrementen el riesgo»**, validado por acta de conformidad de prevención | CP, Cap. 10, restricción no negociable N° 1, que rechaza toda solución que aumente la exposición al riesgo «por buena que sea en todo lo demás». | Si prevención no acepta el checklist ergonómico como evidencia de conformidad, debe acordarse otro instrumento antes de la marcha blanca; la obligación no cambia. | Definir con prevención qué evidencia constituye conformidad — **checklist ergonómico por puesto de terreno**, ya incorporado a los criterios de `RF-PAT-09` y `RF-NAV-11` |
| **J** | Se retiró de RNF-USA-07 la afirmación causal sobre reducción del 22 % de documentación defectuosa, por no ser un hecho declarado por el mandante | El mandante no declara que la validación anticipada reduzca el 22 % de documentación defectuosa. Atribuirle esa causa sería afirmar un hecho que el caso no entrega. | Nulo sobre el alcance. Si el equipo decide comprometer esa meta, debe fundamentarla aparte e incorporarla a la tabla de indicadores. | Si el equipo quiere comprometer esa meta, debe fundamentarla aparte |
| **K** | No se declara **dotación mínima de técnicos por turno** para la presencia en terreno | CP, Cap. 15, RT-21.16 exige presencia en terreno en los tres turnos pero no fija dotación. Declarar un número sin base sería inventar una cifra del CLIENTE. | El dimensionamiento y el costo del acompañamiento quedan abiertos hasta definirlo con operaciones: afecta la Oferta Económica, no el alcance técnico. | Definir con operaciones antes de la entrega final |
| **L** | Mapeo interpretativo de capítulos del BTT a las categorías **Operabilidad** y **Mantenibilidad**, que el BTT no nombra como tales | El CP, Cap. 17.1 nombra las categorías Operabilidad y Mantenibilidad; el BTT organiza esos mismos contenidos por capítulo sin usar esos nombres. El mapeo es interpretativo y se declara como tal. | Si Célula 3 mapea distinto, cambia la ubicación de algunos RNF entre categorías, no su contenido ni su exigibilidad. | Confirmar con Célula 3 |
 
> *Corrección aplicada 2026-09-05: las tablas C.1 y C.2 no traían todas las columnas que el CP, Cap. 17.1 exige para el Registro de Supuestos —«con su fundamento, su impacto si resulta equivocada y la instancia en que se validará»—. C.1 carecía de **Fundamento**, que existía en la Decisión N° 1 §11 y se perdió al copiar: se recupera desde allí. C.2 carecía de **Fundamento** e **Impacto**, que se redactan a partir de las bases citadas en cada fila. Los supuestos del bloque C.3 (M a T) ya los traían en prosa.*
...
**Validación.** Acta de conformidad de prevención de riesgos y del sindicato antes del diseño detallado; prueba en equipo real durante la marcha blanca. **Vinculación:** `RF-PAT-09`.
...
La Decisión N° 19 mantiene el VMS y la visualización de cámaras existentes. No se crea un portal de video ni un RF de CCTV sin una conducta confirmada en las bases. La arquitectura podrá prever un adaptador de integración para eventos, salud o evidencia del VMS; sus contratos concretos deberán validarse con el CLIENTE y Célula 3.
...
| C | Supuestos declarados | **25** — S1 a S5, A a L, M a T |
...
| **Decisión N° 6 ampliada** | Se incorpora mecanismo de **cola virtual** (notificación proactiva al transportista para retener el camión en origen) y se agrega Arica (Ramírez-Nafarrate et al. 2017) como fuente de verificación externa, en A.1, A.3 y meta 14 |
| **Cruce con Célula 1 (Cap. 2.8)** | Los 5 supuestos de su documento de Comprensión del Problema coincidían con decisiones ya adoptadas por esta célula (N° 1, 2, 4, 6 y 8) y fueron comunicados a Fran y Fabi para su remoción de Subdocumento 2, por infringir la separación estricta entre problema y solución que exige el checklist del curso. Al verificar el fundamento de la Restricción N° 11 (Decisión N° 1) y el calendario de captura del conocimiento tácito (Decisión N° 4) contra `01_decision_01_tos_2012_registro_final.md`, ambos ya estaban resueltos ahí con más detalle que en el cruce inicial — se corrige la síntesis de la Decisión N° 4 en A.3 para remitir a esa resolución (Pendiente 6) en vez de duplicarla de forma más pobre |
| **Corrección Plan Maestro 2026-09-04** | Se amplía `RF-CON` a 14; se hace indivisible el compromiso de alianza 2029; se separan exactitud de posición y pendientes por verificar; se corrigen la interpretación del congelamiento, CCTV y la Decisión N° 20 sin presumir un traslado fuera del terminal |
...
| `complemento_reglas_negocio_y_rf_pat_10.md` | Su supuesto de vía de excepción queda incorporado aquí como **supuesto O** y hoy se materializa en `RF-PAT-09`; sus reglas, en `Registro reglas de negocio v2.md`. El nombre histórico del archivo no altera el ID vigente |
